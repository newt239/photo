...
export const TimelineGeotagPanel = ({ photos }: Props) => {
  ...
  const matches = useMemo(() => {
    ...
    const toleranceMs = Number(toleranceMinutes) * 60_000;
  ...
  return (
    <Stack gap="md">
      ...
      <Group grow align="flex-start">
        <Select
          ...
          data={[
            { label: "±15 分", value: "15" },
            { label: "±1 時間", value: "60" },
            { label: "±3 時間", value: "180" },
            { label: "±12 時間", value: "720" },
          ]}
          value={toleranceMinutes}
          onChange={(value) => setToleranceMinutes(value ?? "60")}
          allowDeselect={false}
        />
        <NumberInput
          label="撮影時刻の補正（時間）"
      ...
      {timeline && (
        <Stack gap="sm">
          <Text size="sm">
            {matches.length} 件の写真が照合できました（選択中 {selected.length} 件）。
          </Text>
          {matches.length > 0 && (
