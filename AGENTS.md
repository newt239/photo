...
Node は `package.json` の `devEngines.runtime`、pnpm は `packageManager` で固定します。`mise.toml` や `.node-version` は使いません。
...
- アクセス判定の順序: (1) オーナー本人なら常に可、(2) 公開アルバムに属していれば可、いずれでもなければ不可
